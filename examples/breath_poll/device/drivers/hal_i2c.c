...
/* Initialize I2C master mode, Initialization failure return false, Initialization success return true. */
bool I2C_InitMaster(I2C_Type * I2Cx, I2C_Master_Init_Type * init)
{
    ...
    I2Cx->CR |= I2C_CR_RESTART_MASK     /* Set restart. */
...
void I2C_InitSlave(I2C_Type * I2Cx, I2C_Slave_Init_Type * init)
{
    I2Cx->ENR &= ~I2C_ENR_ENABLE_MASK;  /* Disable I2C. */
    I2Cx->CR &= ~(I2C_CR_DISSLAVE_MASK | I2C_CR_MASTER_MASK | I2C_CR_SLAVE10_MASK);
    I2Cx->SAR = I2C_SAR_ADDR((uint8_t)init->SlaveAddr);
    I2Cx->CR |= I2C_CR_REPEN_MASK;
    I2Cx->IMR = 0u;
}
...
void I2C_ClearInterruptStatus(I2C_Type * I2Cx, uint32_t interrupts)
{
    ...
    if ( (I2C_INT_READREQ & interrupts) != 0u)
    {
        I2Cx->RDREQ;
    }
    if ( (I2C_INT_RX_DONE & interrupts) != 0u)
    {
        I2Cx->RXDONE;
    }
}
...
/* I2C slave module put data. */
void I2C_SlaveWriteData(I2C_Type * I2Cx, I2C_SlaveXfer_Type * xfer)
{
    if ( 0u != (I2Cx->RAWISR & I2C_INT_READREQ) )
    {
        I2C_ClearInterruptStatus(I2Cx, I2C_INT_TX_ABORT | I2C_INT_READREQ);
        I2C_PutData(I2Cx, xfer->XferBuf[xfer->TxIdx++]);
    }

    if (xfer->TxIdx == xfer->BufLen)
    {
        xfer->TxIdx = 0u;
    }
}

/* I2C slave module get data. */
void I2C_SlaveReadData(I2C_Type * I2Cx, I2C_SlaveXfer_Type * xfer)
{
    if ( 0u != (I2C_GetStatus(I2Cx) & I2C_STATUS_RX_NOTEMPTY) )
    {
        xfer->XferBuf[xfer->RxIdx++] = I2C_GetData(I2Cx);
    }
    if (xfer->RxIdx == xfer->BufLen)
    {
        xfer->RxIdx = 0u;
    }
}
...
/* I2C slave handler. */
void I2C_SlaveXferHandler(I2C_Type * I2Cx, I2C_SlaveXfer_Type * xfer, uint32_t interrupts)
{
    if ( 0u != (interrupts & I2C_INT_RX_NOTEMPTY) )
    {
        xfer->XferBuf[xfer->RxIdx++] = I2C_GetData(I2Cx);
    }
    if ( 0u != (interrupts & I2C_INT_READREQ) )
    {
        I2C_ClearInterruptStatus(I2Cx, I2C_INT_TX_ABORT | I2C_INT_READREQ);
        I2C_PutData(I2Cx, xfer->XferBuf[xfer->TxIdx++]);
    }
