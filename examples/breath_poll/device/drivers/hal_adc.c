...
{
    uint32_t tmp32 = ADCx->LDATA;

    *channel = (tmp32 & ADC_LDATA_LCHANNELSEL_MASK ) >> ADC_LDATA_LCHANNELSEL_SHIFT;
    if (flags)
    {
        *flags = (tmp32 & (ADC_LDATA_LOVERRUN_MASK | ADC_LDATA_LVALID_MASK) ) >> ADC_LDATA_LOVERRUN_SHIFT;
    }

    return (tmp32 & ADC_LDATA_LDATA_MASK ) >> ADC_LDATA_LDATA_SHIFT;
