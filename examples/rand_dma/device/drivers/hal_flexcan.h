 ...
 */
#define FLEXCAN_DRIVER_VERSION 1u /*!< FLEXCAN_1. */
...
typedef enum
{
    FLEXCAN_MbSize_8Bytes  = 0u, /*!< 8 bytes data per Message buffer. */
    FLEXCAN_MbSize_16Bytes = 1u, /*!< 16 bytes data per Message buffer. */
    FLEXCAN_MbSize_32Bytes = 2u, /*!< 32 bytes data per Message buffer. */
    FLEXCAN_MbSize_64Bytes = 3u, /*!< 64 bytes data per Message buffer. */
} FLEXCAN_MbSize_Type;

/*!
 * @brief FlexCAN Message Buffer Size.
 */
typedef enum
{
    FLEXCAN_DataLen_None    = 0u, /*!< 0 byte data per Message buffer. */
    FLEXCAN_DataLen_1Byte   = 1u, /*!< 1 byte data per Message buffer. */
    FLEXCAN_DataLen_2Bytes  = 2u, /*!< 2 bytes data per Message buffer. */
    FLEXCAN_DataLen_3Bytes  = 3u, /*!< 3 bytes data per Message buffer. */
    FLEXCAN_DataLen_4Bytes  = 4u, /*!< 4 bytes data per Message buffer. */
    FLEXCAN_DataLen_5Bytes  = 5u, /*!< 5 bytes data per Message buffer. */
    FLEXCAN_DataLen_6Bytes  = 6u, /*!< 6 bytes data per Message buffer. */
    FLEXCAN_DataLen_7Bytes  = 7u, /*!< 7 bytes data per Message buffer. */
    FLEXCAN_DataLen_8Bytes  = 8u, /*!< 8 bytes data per Message buffer. */
    FLEXCAN_DataLen_12Bytes = 9u, /*!< 12 bytes data per Message buffer. */
    FLEXCAN_DataLen_16Bytes = 10u, /*!< 16 bytes data per Message buffer. */
    FLEXCAN_DataLen_20Bytes = 11u, /*!< 20 bytes data per Message buffer. */
    FLEXCAN_DataLen_24Bytes = 12u, /*!< 24 bytes data per Message buffer. */
    FLEXCAN_DataLen_32Bytes = 13u, /*!< 32 bytes data per Message buffer. */
    FLEXCAN_DataLen_48Bytes = 14u, /*!< 48 bytes data per Message buffer. */
    FLEXCAN_DataLen_64Bytes = 15u, /*!< 64 bytes data per Message buffer. */
} FLEXCAN_DataLen_Type;
...
typedef struct
{
    ...
    uint32_t FdBitRate;                   /*!< Data bit per second when using FLEXCAN for transmision and reception. */
    bool EnableFdRate;                    /*!< Enable switch data bitrate. */
} FLEXCAN_FdBitRateConf_Type;
...
/*!
 * @brief This type of structure instance is used to keep the settings when calling the @ref FLEXCAN_InitFd() to initialize the setting of FLEXCAN FD.
 */
typedef struct
{
    FLEXCAN_FdBitRateConf_Type * FdBitRateConf; /*!< FLEXCAN data bitrate and time synchronization setup. */
    FLEXCAN_MbSize_Type  MbSize;         /*!< Indicate data size in FlexCAN Message Buffer. */
    bool EnableIsoCanFd;                 /*!< Enable SwitchBitRate. */
} FLEXCAN_InitFd_Type;
...
/*!
 * @brief This type of structure instance is used to keep the settings when calling the @ref FLEXCAN_WriteFdTxMb() to set the mask for buffer.
 */
typedef struct
{
    struct
    {
        ...
        uint32_t RESERVED_1 : 1;    /*!< Reservation. */
    ...
    union
    {
        struct
        {
            uint32_t WORD[16]; /*!< CAN FD Frame payload word. */
        };
        struct
        {
            /* The sequence refers to the little-endian-storage and big-endian transfer. */
 ...
 * @return 'true' to setup the FLEXCAN module successfully, false to fail to set the bitrate wrongly.
 */
bool FLEXCAN_Init(FLEXCAN_Type * FLEXCANx, FLEXCAN_Init_Type * init);

/*!
 * @brief Set up the FLEXCAN module FD function with the indicated setting.
 *
 * @param FLEXCANx FLEXCAN instance.
 * @param init Pointer to the initialization structure. See to @ref FLEXCAN_InitFd_Type.
 * @return 'true' to setup the FLEXCAN module FD function successfully, false to fail to set the bitrate wrongly.
 */
 ...
 *
 * This would work when people want to change the bitrate after setting up the module.
 *
 * @param FLEXCANx FLEXCAN instance.
...
/*!
 * @brief Set up the FLEXCAN FD timing module with the indicated setting.
 *
 * This would work when people want to change the bitrate after setting up the module.
 *
 * @param FLEXCANx FLEXCAN instance.
...
/*!
 * @brief Set up the FLEXCAN FD module bitrate with the indicated setting.
 *
 * This would work when people want to change the FLEXCAN data bitrate after setting up the module.
 *
 * @param FLEXCANx FLEXCAN instance.
 * @param conf Pointer to the initialization structure. See to @ref FLEXCAN_FdBitRateConf_Type.
 * @return None.
 */
...
/*!
 * @brief Clear the indicated FD MB.
 *
 * Clear the previous information in the buffer.
 *
 * @param FLEXCANx FLEXCAN instance.
...
/*!
 * @brief Set FD message buffer with value 0.
 *
 * Clear the previous information in the buffer.
 *
 * @param FLEXCANx FLEXCAN instance.
...
/*!
 * @brief Write the FD Mb frame into the indicated FD Mb channel.
 *
 * @param FLEXCANx FLEXCAN instance.
 * @param channel The indicated FLEXCAN Mb channel.
 * @param mb The prepared Mb for Tx. See to @ref FLEXCAN_FdMb_Type.
 * @return 'true' to put the Mb frame successfully, 'false' to fail.
 */
...
/*!
 * @brief Read the FD Mb frame from the indicated Mb channel.
 *
 * @param FLEXCANx FLEXCAN instance.
 * @param channel The indicated FLEXCAN Mb channel.
 * @param mb The Mb for Rx. See to @ref FLEXCAN_FdMb_Type.
 * @return 'true' to get the Mb frame successfully, 'false' to fail.
 */
